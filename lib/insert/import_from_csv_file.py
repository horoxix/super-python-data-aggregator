...
def insert_data_from_csv(csv_file, database, table):
    print(database)
    database_object = get_db_type.get_database_type(database)
    print(database_object)
    csv_data = pd.read_csv(csv_file)
    sh_uuid = update_suite_history_table(csv_data, database_object)
    csv_data.drop([0])
    arg_list = []
    row_total = 0
    for row in csv_data.itertuples(index=False):
        ...
        insert_results_into_table(get_table(table), database_object, *arg_list)
        row_total += 1
        arg_list.clear()
...
def update_suite_history_table(csv_data, database_object):
    ...
    insert_results_into_table(get_table('suite_run_history'), database_object, *suite_history_args)
